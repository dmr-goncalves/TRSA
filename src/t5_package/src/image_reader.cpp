...
void imageClbk ( const sensor_msgs::ImageConstPtr& msg)
{
  ...
  cv::imshow( "Image Reader Raw", cvImagePtr->image ); // opens a named window to display the image
  cv::waitKey(1);
}

void ProcessedImageClbk ( const sensor_msgs::ImageConstPtr& msg)
{
  cv_bridge::CvImagePtr cvImagePtr;
  try
  {
  ...
  cv::imshow( "Image Reader Processed", cvImagePtr->image ); // opens a named window to display the image
  cv::waitKey(1);
}
...
int main(int argc, char **argv)
{
  ...
  image_transport::Subscriber ProcessedImageSub = it.subscribe("/camera/image_processed", 1, ProcessedImageClbk);
  ros::spin();
